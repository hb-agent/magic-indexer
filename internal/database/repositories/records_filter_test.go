...
// TestCountByCollectionFiltered pins the P-1 fix: the
// connection-resolver totalCount must reflect the SAME filter
// applied to the SELECT. The prior shape called
// GetCollectionCount(collection) regardless of the request's
// where/authors/labels/search/excludePds args, so filtered
// pages returned wrong totals (e.g. "page 1 of 47" when only 3
// records actually matched).
//
// Failure here would mean the resolver-layer regression has
// been re-introduced.
func TestCountByCollectionFiltered_AuthorsFilterApplied(t *testing.T) {
	db := seedFilterRecords(t)
	ctx := context.Background()

	// Baseline — no filter, returns the full collection count.
	total, err := db.Records.CountByCollectionFiltered(ctx, "col.a", repositories.RecordFilter{}, nil)
	if err != nil {
		t.Fatalf("baseline count: %v", err)
	}
	if total != 4 { // alice:2 + bob:1 + carol:1 per seed
		t.Errorf("unfiltered col.a count = %d, want 4", total)
	}

	// Authors filter — must return only matching DIDs' rows.
	filtered, err := db.Records.CountByCollectionFiltered(ctx, "col.a",
		repositories.RecordFilter{Authors: []string{"did:plc:alice"}}, nil)
	if err != nil {
		t.Fatalf("filtered count: %v", err)
	}
	if filtered != 2 {
		t.Errorf("col.a count filtered to alice = %d, want 2", filtered)
	}

	// Multi-author filter.
	multi, err := db.Records.CountByCollectionFiltered(ctx, "col.a",
		repositories.RecordFilter{Authors: []string{"did:plc:alice", "did:plc:carol"}}, nil)
	if err != nil {
		t.Fatalf("multi-author count: %v", err)
	}
	if multi != 3 {
		t.Errorf("col.a count filtered to alice+carol = %d, want 3", multi)
	}
}

// TestCountByCollectionFiltered_EmptyAuthorsShortCircuits pins the
// load-bearing nil-vs-empty distinction: an empty-but-non-nil
// Authors slice means "match nothing" and returns 0 without a DB
// query. The full-collection unfiltered behaviour requires nil.
func TestCountByCollectionFiltered_EmptyAuthorsShortCircuits(t *testing.T) {
	db := seedFilterRecords(t)
	ctx := context.Background()

	got, err := db.Records.CountByCollectionFiltered(ctx, "col.a",
		repositories.RecordFilter{Authors: []string{}}, nil)
	if err != nil {
		t.Fatalf("empty authors count: %v", err)
	}
	if got != 0 {
		t.Errorf("empty authors slice should return 0, got %d", got)
	}
}

// TestCountByCollectionFiltered_NoFiltersUsesFastPath pins that the
// no-filter case delegates to GetCollectionCount (same value, no
// WHERE-builder overhead).
func TestCountByCollectionFiltered_NoFiltersUsesFastPath(t *testing.T) {
	db := seedFilterRecords(t)
	ctx := context.Background()

	got, err := db.Records.CountByCollectionFiltered(ctx, "col.a", repositories.RecordFilter{}, nil)
	if err != nil {
		t.Fatalf("count: %v", err)
	}
	baseline, err := db.Records.GetCollectionCount(ctx, "col.a")
	if err != nil {
		t.Fatalf("baseline: %v", err)
	}
	if got != baseline {
		t.Errorf("filtered-with-no-filters %d != baseline %d", got, baseline)
	}
}
